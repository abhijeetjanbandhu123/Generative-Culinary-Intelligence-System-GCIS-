...
function Scanner({ addItemsToPantry, setActiveTab, apiConfigured }) {
  ...
  const sendToBackend = async (base64Image) => {
    ...
    try {
      ...
      setErrorMsg('Scanning analysis failed. Please verify API configuration or try again.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', gap: '1.5rem', marginBottom: '4px' }}>
              <button 
                style={{
                  ...
                  border: 'none',
                  borderBottom: inputMode === 'visual' ? '2px solid var(--primary)' : '2px solid transparent',
                  color: inputMode === 'visual' ? 'var(--primary)' : 'var(--text-muted)',
                  paddingBottom: '10px',
                  fontWeight: 600,
                  fontSize: '0.9rem',
                  cursor: 'pointer'
              ...
              <button 
                style={{
                  background: 'transparent',
                  border: 'none',
                  borderBottom: inputMode === 'form' ? '2px solid var(--primary)' : '2px solid transparent',
                  color: inputMode === 'form' ? 'var(--primary)' : 'var(--text-muted)',
                  paddingBottom: '10px',
                  fontWeight: 600,
                  fontSize: '0.9rem',
                  cursor: 'pointer'
